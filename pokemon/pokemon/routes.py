...
from pokemon.model import Pokemon, Type, User
...
@pokemon_bp.route('/')
def index():
    querr = db.select(Pokemon).where(Pokemon.user == current_user)
    pokemons= db.session.scalars(querr).all()
...
def new_pokemon():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Add new pokemon successfull!', 'success')
